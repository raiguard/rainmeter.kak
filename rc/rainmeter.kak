...
provide-module rainmeter %{
    ...
    add-highlighter shared/rainmeter/code/keyword_alias regex (?i)^\s*(alias)\s*=\s*(cpu|gpu|io|ioread|iowrite|ram|ramshared|vram|vramshared)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_barorientation regex (?i)^\s*(barorientation)\s*=\s*(horizontal|vertical)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_bitmapalign regex (?i)^\s*(bitmapalign)\s*=\s*(center|left|right)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_bothimageflip regex (?i)^\s*(bothimageflip)\s*=\s*(both|horizontal|none|vertical)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_channel regex (?i)^\s*(channel)\s*=\s*(0|1|2|3|4|5|6|7|avg|bl|br|c|fl|fr|l|lfe|r|sl|sr|sub|sum)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_coretemptype regex (?i)^\s*(coretemptype)\s*=\s*(busmultiplier|busspeed|corebusmultiplier|corespeed|cpuname|cpuspeed|load|maxtemperature|power|tdp|temperature|tjmax|vid)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_datetype regex (?i)^\s*(datetype)\s*=\s*(accessed|created|modified)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_graphorientation regex (?i)^\s*(graphorientation)\s*=\s*(horizontal|vertical)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_graphstart regex (?i)^\s*(graphstart)\s*=\s*(left|right)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_iconsize regex (?i)^\s*(iconsize)\s*=\s*(extralarge|large|medium|small)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_imageflip regex (?i)^\s*(imageflip)\s*=\s*(both|horizontal|none|vertical)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_infotype regex (?i)^\s*(infotype)\s*=\s*(filecount|foldercount|foldersize)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_maskimageflip regex (?i)^\s*(maskimageflip)\s*=\s*(both|horizontal|none|vertical)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_measure regex (?i)^\s*(measure)\s*=\s*(calc|cpu|freediskspace|loop|mediakey|memory|netin|netout|nettotal|nowplaying|physicalmemory|plugin|recyclemanager|registry|script|string|swapmemory|sysinfo|time|uptime|webparser)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_meter regex (?i)^\s*(meter)\s*=\s*(bar|bitmap|button|histogram|image|line|rotator|roundline|shape|string)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_outputtype regex (?i)^\s*(outputtype)\s*=\s*(ansi|utf16|utf8)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_playername regex (?i)^\s*(playername)\s*=\s*(aimp|cad|itunes|mediamonkey|spotify|winamp|wlm|wmp)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_playertype regex (?i)^\s*(playertype)\s*=\s*(album|artist|cover|duration|file|genre|lyrics|number|position|progress|rating|repeat|shuffle|state|status|title|volume|year)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_port regex (?i)^\s*(port)\s*=\s*(input|output)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_powerstate regex (?i)^\s*(powerstate)\s*=\s*(acline|hz|lifetime|mhz|percent|status|status2)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_primaryimageflip regex (?i)^\s*(primaryimageflip)\s*=\s*(both|horizontal|none|vertical)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_recycletype regex (?i)^\s*(recycletype)\s*=\s*(count|size)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_reghkey regex (?i)^\s*(reghkey)\s*=\s*(hkey_classes_root|hkey_current_config|hkey_current_user|hkey_dyn_data|hkey_local_machine|hkey_performance_data)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_rescounttype regex (?i)^\s*(rescounttype)\s*=\s*(gdi|handle|user|window)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_secondaryimageflip regex (?i)^\s*(secondaryimageflip)\s*=\s*(both|horizontal|none|vertical)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_sortdatetype regex (?i)^\s*(sortdatetype)\s*=\s*(accessed|created|modified)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_sorttype regex (?i)^\s*(sorttype)\s*=\s*(date|name|size|type)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_speedfanscale regex (?i)^\s*(speedfanscale)\s*=\s*(c|f|k)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_speedfantype regex (?i)^\s*(speedfantype)\s*=\s*(fan|temperature|voltage)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_state regex (?i)^\s*(state)\s*=\s*(hide|maximized|minimized|show)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_stringalign regex (?i)^\s*(stringalign)\s*=\s*(center|centerbottom|centercenter|centertop|left|leftbottom|leftcenter|lefttop|right|rightbottom|rightcenter|righttop)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_stringcase regex (?i)^\s*(stringcase)\s*=\s*(lower|none|proper|upper)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_stringeffect regex (?i)^\s*(stringeffect)\s*=\s*(border|none|shadow)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_stringstyle regex (?i)^\s*(stringstyle)\s*=\s*(bold|bolditalic|italic|normal)$ 1:keyword 2:module
    ...
    add-highlighter shared/rainmeter/code/keyword_traymeter regex (?i)^\s*(traymeter)\s*=\s*(bitmap|histogram)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_transformstroke regex (?i)^\s*(transformstroke)\s*=\s*(fixed|normal)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_type regex (?i)^\s*(type)\s*=\s*(0|1|band|bandfreq|deviceid|devicelist|devicename|devicestatus|fft|fftfreq|filecount|filedate|filename|filepath|filesize|filetype|foldercount|folderpath|foldersize|format|icon|pathtofile|peak|rms)$ 1:keyword 2:module
    add-highlighter shared/rainmeter/code/keyword_wifiinfotype regex (?i)^\s*(wifiinfotype)\s*=\s*(auth|encryption|list|phy|quality|ssid)$ 1:keyword 2:module
}
